...
% legend('$\zeta_{11}$', '$\zeta_{21}$', '$\zeta_{31}$', '$\zeta_{41}$', 'NumColumns', 1, 'interpreter','latex', 'FontSize', 12);
% xlim([0,50]);
% set(gca, 'FontSize', 16)
% 
% subplot(3,2,3)
...
%     hat_Delta_STC5(i,:) = (d_delta_Delta_eta{5}(i,:) + delta_Delta_eta{5}(i,:));
% 
%     eta_est(i,:) = eta_{5}(i,:) + hat_Delta_STC5(i,:);
% end
% figure(7)
...
% % 估计误差
% for i=2:5
%     for j=1:size(delta_eta{i},1)
%         estimation_err1{i}(j) = norm(delta_eta{i}(j,:) - eta_{i}(j,:));
...
% hold on
% 
% legend('$\sum_{i=1}^{4}\frac{\|\eta_{ei1}\|+\|\eta_{ei2}\|+\|\eta_{ei3}\|}{4}$',...
%     '$\sum_{i=1}^{4}\frac{\|\omega_{ei1}\|+\|\omega_{ei2}\|+\|\omega_{e_i3}\|}{4}$',...
%     '$\sum_{i=1}^{4}\frac{\|\tilde{\delta}_{\breve{\eta}_{i}}\|+\|\tilde{\delta}_{\Delta \eta_{i}}\|+\|\tilde{\delta}_{\breve{\omega}_{i}}\|+\|\tilde{\delta}_{\Delta \omega_{i}}\|}{4}$',...
%     'Interpreter','latex')
